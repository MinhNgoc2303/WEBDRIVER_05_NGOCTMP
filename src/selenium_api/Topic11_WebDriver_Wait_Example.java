...
public class Topic11_WebDriver_Wait_Example {
	...
	public void TS02_ExiplictWait() {
		...
		/* presence */
		waitExplicit.until(
				ExpectedConditions.presenceOfElementLocated(By.xpath("//div[@id='ctl00_ContentPlaceholder1_Panel1']")));

		/* Visible */
		waitExplicit.until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//div[@id='ctl00_ContentPlaceholder1_Panel1']")));

		// Step 03 - Print day selected (Before AJAX call) -> present not select = "No
		...
		// must be check it have class change before and after selected
		waitExplicit.until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//a[text()='20']/parent::td[@class='rcSelected']")));
		Assert.assertTrue(
				driver.findElement(By.xpath("//a[text()='20']/parent::td[@class='rcSelected']")).isDisplayed());
